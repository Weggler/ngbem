...
for order in range(1,4):
    # uniform refinement
    for k in range(5):
        mesh = Mesh(OCCGeometry(shape).GenerateMesh(maxh=0.5 ** (k + 1))).Curve(order)
        ...
        uexa = CF(1. / sqrt((x-2)**2 + (y-2)**2 + (z-2)**2))
        ...
        sol = GMRes(A=lhs, b=rhs, pre=pre, maxsteps=300, printrates=False)
        ...
        fes_screen = H1(mesh_screen, order=4)
        ...
        print(order, " ", fesL2.ndof, " ", fesH1.ndof, " ", errd, " ", errn, " ", errs)
